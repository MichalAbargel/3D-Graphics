...
public class Render {
    ...
    private Color calcColor(List<Ray> rays){
        ...
        Color C3= _rayTracerBase.traceRay(rays.get(_camera.SuperSamplingNumber*(_camera.SuperSamplingNumber-1)));
        Color C4= _rayTracerBase.traceRay(rays.get(_camera.SuperSamplingNumber*_camera.SuperSamplingNumber-1));
        if(C1==C2 && C1==C3 && C1==C4)
            return C1;//all the colors are the same.
        Color pixelColor=Color.BLACK;
        for (int i=0; i<rays.size(); i++){
        pixelColor=pixelColor.add(_rayTracerBase.traceRay(rays.get(i)));
        }
    ...
    public void renderImage() {
        ...
        if (threadsCount == 0)
            for (int i = 0; i < nY; ++i)
                for (int j = 0; j < nX; ++j){
                    //*****For other option with out superSampling:*****
                    //List<Ray> rays=List.of(_camera.constructRayThroughPixel(nX,nY,i, j));
                    //*******
    ...
    private void castRay(int nX, int nY, int row, int col) {
        List<Ray> rays=_camera.constructRayThroughPixelSuperSampling(nX,nY,row, col);
        Color color = this.calcColor(rays);
        ...
        for (int i = threadsCount - 1; i >= 0; --i) {
            threads[i] = new Thread(() -> {
                ...
                while (thePixel.nextPixel(pixel)){
                    //get color of pixel through the rays pixel
                    this.castRay(nX,nY,pixel.row, pixel.col);
